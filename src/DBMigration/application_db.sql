...
IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE name = N'$(ApplicationDatabaseUser)')
CREATE LOGIN [$(ApplicationDatabaseUser)] WITH PASSWORD=N'$(ApplicationDatabasePwd)', DEFAULT_DATABASE=[$(ApplicationDatabaseName)], CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF
ELSE
ALTER LOGIN [$(ApplicationDatabaseUser)] WITH PASSWORD=N'$(ApplicationDatabasePwd)', DEFAULT_DATABASE=[$(ApplicationDatabaseName)], CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF
GO

USE [$(ApplicationDatabaseName)]
GO

IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'$(ApplicationDatabaseUser)')
CREATE USER [$(ApplicationDatabaseUser)] FOR LOGIN [$(ApplicationDatabaseUser)]
GO

EXEC sp_addrolemember N'db_owner', N'$(ApplicationDatabaseUser)'
GO
