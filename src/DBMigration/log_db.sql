...
IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE name = N'$(LoggingDatabaseUser)')
CREATE LOGIN [$(LoggingDatabaseUser)] WITH PASSWORD=N'$(LoggingDatabasePwd)', CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF
ELSE
ALTER LOGIN [$(LoggingDatabaseUser)] WITH PASSWORD=N'$(LoggingDatabasePwd)', CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF
GO

USE [$(LoggingDatabaseName)]
GO

IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = N'$(LoggingDatabaseUser)')
CREATE USER [$(LoggingDatabaseUser)] FOR LOGIN [$(LoggingDatabaseUser)]
GO

EXEC sp_addrolemember N'db_owner', N'$(LoggingDatabaseUser)'
GO
